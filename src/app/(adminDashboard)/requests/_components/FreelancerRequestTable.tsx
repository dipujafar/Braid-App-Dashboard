...
import { useGetAllFreelancerRequestsQuery } from "@/redux/api/freelancerRequestApi";
...
const FreelancerRequestTable = () => {
    ...
    const { data: freelancerRequests, isLoading } = useGetAllFreelancerRequestsQuery(queries);
    ...
    return (
        <div className="bg-[#F9F9FA] rounded-2xl">
            ...
            <DataTable columns={columns} data={freelancerRequests?.data} pageSize={Number(limit)} total={freelancerRequests?.meta?.totalDoc} ></DataTable>
            <RequestModal open={open} setOpen={setOpen} data={currentData} title="Freelancer" ></RequestModal>
        </div>
    );
};

export default FreelancerRequestTable;
