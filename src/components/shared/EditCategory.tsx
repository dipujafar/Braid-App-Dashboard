...
const EditSubCategory = ({ open, setOpen, data }: TPropsType) => {
    const [form] = Form.useForm();
    const [editSubCategory, { isLoading }] = useEditSubCategoriesMutation();
    const [currentData, setCurrentData] = useState<any>();

    useEffect(() => {
        setCurrentData(data);
        form.setFieldsValue({ categoryName: data?.name });
    }, [data]);
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            await editSubCategory({ id: data?._id, data: formData }).unwrap();
            toast.success("Successfully updated category", { duration: 1000 });
            form.resetFields();
            setOpen(false);
        }
        catch (error: any) {
        ...
        <>
            ...
            >
                <div className="py-14">
                    ...
                    <h4 className=" text-2xl font-medium text-center">Edit Sub Category</h4>
                    ...
                    >
                        <Form
                            ...
                            layout="vertical"
                            initialValues={{ categoryName: currentData?.name }}
                            style={{
                                maxWidth: 500,
                                marginTop: "25px",
                            }}
