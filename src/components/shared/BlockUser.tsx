"use client";
import { useUserStatusChangeMutation } from '@/redux/api/usersApi';
...
export default function BlockUser({ id, isActive }: any) {
    const [userChangeStatus] = useUserStatusChangeMutation();
    ...
    const confirmBlock: PopconfirmProps["onConfirm"] = async () => {
        try {
            if (isActive) {
                ...
                await userChangeStatus({ id, data: { status: "ongoing" } }).unwrap();
